...
from getmailcore.sslwrappers import *
...
class POP3SSLinitMixIn(object):
    ...
    def _connect(self):
        ...
        try:
            self.log.trace(
                'establishing POP3 SSL connection to %(server)s:%(port)d with '
                'certificatemode %(certificatemode)s, keyfile %(keyfile)s, '
                'certfile %(certfile)s, cacertificatesfile '
                '%(cacertificatesfile)s',
                self.conf
            )
            self.conn = Pop3Ssl(
                ...
                caCertificatesFile=self.conf['cacertificatesfile'],
                certificateMode=self.conf['certificatemode']
            )
        except poplib.error_proto, o:
            raise getmailOperationError('POP error (%s)' % o)
        except socket.timeout:
...
class IMAPSSLinitMixIn(object):
    ...
    def _connect(self):
        ...
        try:
            self.log.trace(
                'establishing IMAP4 SSL connection to %(server)s:%(port)d with '
                'certificatemode %(certificatemode)s, keyfile %(keyfile)s, '
                'certfile %(certfile)s, cacertificatesfile '
                '%(cacertificatesfile)s',
                self.conf
...
class RetrieverSkeleton(ConfigurableBase):
    ...
    def _read_oldmailfile(self):
        ...
        try:
            ...
            self.log.moreinfo('no oldmail file for %s' % self)
    ...
    def write_oldmailfile(self, forget_deleted=True):
        ...
        try:
            ...
            msgids = set(self.__delivered).union(set(self.oldmail))
            for msgid in msgids:
...
class POP3RetrieverBase(RetrieverSkeleton):
    ...
    def _getmsgbyid(self, msgid):
        self.log.debug('msgid %s' % msgid)
        msgnum = self._getmsgnumbyid(msgid)
        self.log.debug('msgnum %i' % msgnum)
        try:
...
class MultidropPOP3RetrieverBase(POP3RetrieverBase):
    ...
    def _getmsgbyid(self, msgid):
        ...
        msg.recipient = stripAddress(line.strip())
        return msg
...
class IMAPRetrieverBase(RetrieverSkeleton):
    ...
    def _parse_imapcmdresponse(self, cmd, *args):
        ...
        if cmd.lower().startswith('login'):
            self.log.debug('login command response %s' % resplist)
        else:
            self.log.debug(
    ...
    def _selectmailbox(self, mailbox):
        ...
        if self.mailbox is not None:
            ...
            self.conn.close()
        self.log.debug('selecting mailbox "%s"' % mailbox)
        try:
            response = self._parse_imapcmdresponse('SELECT', mailbox)
            count = int(response[-1]) # use *last* EXISTS response
    ...
    def _delmsgbyid(self, msgid):
        ...
        try:
            ...
            self.log.debug('deleting message "%s"' % uid)
            response = self._parse_imapuidcmdresponse(
        ...
        try:
            ...
            for msgid in self.oldmail:
                ...
                if msgid not in self.msgsizes and age > VANISHED_AGE:
                    self.log.debug('removing vanished old message id %s' 
                                   % msgid)
                    del self.oldmail[msgid]
        except imaplib.IMAP4.error, o:
            raise getmailOperationError('IMAP error (%s)' % o)
