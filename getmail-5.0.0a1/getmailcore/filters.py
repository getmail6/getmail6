    ...
    def filter_message(self, msg, retriever):
        ...
        if exitcode in self.exitcodes_drop:
            ...
            if self.conf['ignore_stderr']:
                self.log.info('filter %s: %s' % (self, err))
            else:
                raise getmailFilterError('filter %s returned %d (%s)'
                                         % (self, exitcode, err))
...
class Filter_external(FilterSkeleton, ForkingBase):
    ...
    def initialize(self):
        ...
        try:
            ...
            if set(self.exitcodes_keep).intersection(set(self.exitcodes_drop)):
    ...
    def showconf(self):
        self.log.trace('')
        self.log.info('Filter_external(%s)' % self._confstring())
    ...
    def _filter_command(self, msg, msginfo, stdout, stderr):
        try:
            ...
            changeUserGroup(None, self.conf['user'], self.conf['group'])
    ...
    def _filter_message(self, msg):
        ...
        self.log.debug('command %s %d exited %d' % (self.conf['command'],
                                                      childpid, exitcode))
...
class Filter_classifier(Filter_external):
    ...
    def showconf(self):
        self.log.trace('')
        self.log.info('Filter_classifier(%s)' % self._confstring())
...
class Filter_TMDA(FilterSkeleton, ForkingBase):
    ...
    def showconf(self):
        self.log.trace('')
        self.log.info('Filter_TMDA(%s)' % self._confstring())
    ...
    def _filter_message(self, msg):
        self.log.trace('')
        self._prepare_child()
        if msg.recipient is None or msg.sender is None:
            raise getmailConfigurationError(
