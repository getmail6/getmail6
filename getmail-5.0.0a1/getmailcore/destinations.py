...
class DeliverySkeleton(ConfigurableBase):
    ...
    def retriever_info(self, retriever):
        self.log.trace('')
        if self.REQUIRES_MULTIDROP and not retriever.IS_MULTIDROP:
            raise getmailConfigurationError(
                'Destination %s requires a multidrop retriever class' % self
            )
...
class Maildir(DeliverySkeleton, ForkingBase):
    ...
    def showconf(self):
        self.log.info('Maildir(%s)' % self._confstring())
    ...
    def __deliver_message_maildir(self, uid, gid, msg, delivered_to, received,
        ...
        try:
            ...
            # to detect it
            self.log.error('maildir delivery process failed (%s)', o)
            os._exit(127)
    ...
    def _deliver_message(self, msg, delivered_to, received):
        ...
        user = self.conf['user']
        if os.name == 'posix':
            if user and getUid(user) != os.geteuid():
        ...
        self.log.debug('spawned child %d' % childpid)
...
class Mboxrd(DeliverySkeleton, ForkingBase):
    ...
    def showconf(self):
        self.log.info('Mboxrd(%s)' % self._confstring())
    ...
    def __deliver_message_mbox(self, uid, gid, msg, delivered_to, received,
        ...
        try:
            ...
            # Open and lock mbox file, refusing to create it if it doesn't exist
            mbox = getmailcore.utilities.lockingFile(self.conf['path'], 'r+b', 
                                                     self.conf['locktype'])
            fd = mbox.fileno()
            try:
                status_old = os.fstat(fd)
                # Check if it _is_ an mbox file.  mbox files must start with 
                # "From " in their first line, or are 0-length files.
                mbox.seek(0, os.SEEK_SET)
                first_line = mbox.readline()
                ...
                mbox.write(
                    msg.flatten(delivered_to, received, include_from=True,
                                mangle_from=True) 
                    + os.linesep
                )
                mbox.flush()
                ...
                if not mbox.closed:
                    ...
                    mbox.truncate(status_old.st_size)
                raise getmailDeliveryError(
                    'failure writing message to mbox file "%s" (%s)'
                    % (self.conf['path'], o)
                )
            else:
                # Indicate new mail to MUAs that read mbox: reset atime to 
                # previous value, leave mtime as newer.
                status_new = os.stat(self.conf['path'])
                try:
                    os.utime(self.conf['path'], 
                             (status_old.st_atime, status_new.st_mtime))
                except OSError, o:
                    ...
                    self.log.warning('failed to updated mtime/atime of mbox %s' 
                                     % self.conf['path'])
            finally:
                if not mbox.closed:
                    mbox.close()
            ...
            # Child process; any error must cause us to exit nonzero for parent
            # to detect it
            self.log.error('mbox delivery process failed (%s)', o)
            os._exit(127)
...
class MDA_qmaillocal(DeliverySkeleton, ForkingBase):
    ...
    def showconf(self):
        self.log.info('MDA_qmaillocal(%s)' % self._confstring())
    ...
    def _deliver_qmaillocal(self, msg, msginfo, delivered_to, received, stdout,
        ...
        try:
            ...
            self.log.debug('about to execl() with args %s' % str(args))
            # Modify message
            ...
            changeUserGroup(self.log, self.conf['user'], self.conf['group'])
            ...
            # Child process; any error must cause us to exit nonzero for parent
            # to detect it
            self.log.error('exec of qmail-local failed (%s)', o)
            os._exit(127)
    ...
    def _deliver_message(self, msg, delivered_to, received):
        ...
        self.log.debug('qmail-local %d exited %d' % (childpid, exitcode))
...
class MDA_external(DeliverySkeleton, ForkingBase):
    ...
    def showconf(self):
        self.log.info('MDA_external(%s)' % self._confstring())
    ...
    def _deliver_command(self, msg, msginfo, delivered_to, received,
        ...
        try:
            ...
            # Child process; any error must cause us to exit nonzero for parent
            # to detect it
            self.log.error('exec of command %s failed (%s)', 
                           self.conf['command'], o)
            os._exit(127)
    ...
    def _deliver_message(self, msg, delivered_to, received):
        ...
        self.log.debug('msginfo "%s"' % msginfo)
...
class MultiDestinationBase(DeliverySkeleton):
    ...
    def _get_destination(self, path):
        ...
        if p.startswith('[') and p.endswith(']'):
            ...
            self.log.debug('  getting destination for %s' % path)
            destination_type = self.conf['configparser'].get(destsectionname,
                                                             'type')
            self.log.debug('    type="%s"' % destination_type)
            ...
            for (name, value) in self.conf['configparser'].items(destsectionname):
                if name in ('type', 'configparser'):
                    continue
                self.log.debug('    parameter %s="%s"' % (name, value))
...
class MultiDestination(MultiDestinationBase):
    ...
    def initialize(self):
        ...
        dests = [expandUserVars(item) for item in self.conf['destinations']]
        for item in dests:
            try:
    ...
    def showconf(self):
        self.log.info('MultiDestination(%s)' % self._confstring())
...
class MultiSorter(MultiSorterBase):
    ...
    def showconf(self):
        self.log.info('MultiSorter(%s)' % self._confstring())

    def _deliver_message(self, msg, delivered_to, received):
        self.log.trace('')
        matched = []
        if msg.recipient is None and self.targets:
            raise getmailConfigurationError(
        ...
        if not matched:
            if self.targets:
                ...
                self.log.debug('using default %s' % self.default)
...
class MultiGuesser(MultiSorterBase):
    ...
    def showconf(self):
        self.log.info('MultiGuesser(%s)' % self._confstring())
    ...
    def _deliver_message(self, msg, delivered_to, received):
        ...
        for fields in fieldnames:
            for field in fields:
                self.log.debug(
                    'looking for addresses in %s header fields' % field
                )
                header_addrs.extend(
