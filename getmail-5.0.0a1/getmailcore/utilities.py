...
def className(obj):
    """Given an object, return a string representation of its class name.
    """
    return repr(obj).split()[0].split('.')[-1]


class lockingFile(object):
    WRAPPED_METHODS = ('closed', 'fileno', 'flush', 'read', 'readline', 
                       'readlines', 'seek', 'truncate', 'write')
    def __init__(self, path, mode, locktype):
        assert locktype in ('lockf', 'flock'), 'unknown lock type %s' % locktype
        if locktype == 'lockf':
            self._lockfunc = fcntl.lockf
        elif locktype == 'flock':
            self._lockfunc = fcntl.flock
        self.file = open(path, mode)
        self._lockfunc(self.file, fcntl.LOCK_EX)
        
        for method in self.WRAPPED_METHODS:
            setattr(self, method, getattr(self.file, method))
    
    def close(self):
        self.file.flush()
        self._lockfunc(self.file, fcntl.LOCK_UN)
        self.file.close()
        for method in self.WRAPPED_METHODS:
            delattr(self, method)
        del self.file
    ...
    def __del__(self):
        if hasattr(self, 'file'):
            raise ValueError('failed to call close()')




#######################################
def lockFile(file, locktype):
    """Do file locking."""
    assert locktype in ('lockf', 'flock'), 'unknown lock type %s' % locktype
    if locktype == 'lockf':
...
#######################################
def safeOpen(path, mode, permissions=0600):
...
#######################################
def deliverMaildir(maildirpath, data, hostname, dcount=None, filemode=0600):
    ...
    # Open file to write
    try:
        f = safeOpen(fname_tmp, 'wb', filemode)
        f.write(data)
        f.flush()
        os.fsync(f.fileno())
        f.close()
...
#######################################
def changeUserGroup(logger=None, user=None, _group=None):
    """
    ...
    if _group:
        if logger:
            logger.debug('Getting GID for specified group %s' % _group)
        try:
    ...
    if user:
        if logger:
            logger.debug('Getting UID for specified user %s' % user)
        uid = getUid(user)

    changeUidGid(logger, uid, gid)

#######################################
def changeUidGid(logger=None, uid=None, gid=None):
    """
    ...
    try:
        if gid:
            if os.getegid() != gid:
                if logger:
                    logger.debug('Setting egid to %d' % gid)
                os.setregid(gid, gid)
        if uid:
            if os.geteuid() != uid:
                if logger:
                    logger.debug('Setting euid to %d' % uid)
                os.setreuid(uid, uid)
    except OSError, o:
...
# compatibility with code written against getmail v.4
is_maildir = isMaildir
deliver_maildir = deliverMaildir
