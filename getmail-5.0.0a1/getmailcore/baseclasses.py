...
class ConfigEnumeration:
    """A simple class for making use of named enumeration constants in a config
    file more convenient.
    
    Define individual values with all-caps attribute names at the class level,
    e.g.
    VALUE1 = 'asdf'
    VALUE2 = 'qwer'
    Accesses can then be made case-insensitively by users.
    """
    @classmethod
    ...
    @classmethod
    def allValues(cls):
        r = sorted(attr for attr in dir(self) if attr == attr.upper())


class ConfItem(object):
    securevalue = False
    def __init__(self, name, dtype, default=None, required=True):
        self.log = getmailcore.logging.getLogger(
            getmailcore.utilities.className(self)
        )
        self.name = name
        self.dtype = dtype
    ...
    def validate(self, configuration, val=None):
        ...
        if type(val) is not self.dtype and val != self.default:
            ...
            try:
                if self.dtype == bool:
                    val = getmailcore.utilities.asBool(val)
                else:
...
class ConfDirectory(ConfString):
    ...
    def validate(self, configuration):
        ...
        if val is None:
            return None
        val = getmailcore.utilities.expandUserVars(val)
...
class ConfigurableBase(object):
    ...
    def __init__(self, **args):
        ...
        for (name, value) in args.items():
            ...
            if name.lower() == 'password':
                self.log.trace('setting %s to * (%s)' % (name, type(value)))
            else:
    ...
    def checkconf(self):
        ...
        for item in self._confitems:
            ...
            self.log.trace('checking %s' % item.name)
            self.conf[item.name] = item.validate(self.conf)
        unknown_params = set(self.conf) - set(
            [item.name for item in self._confitems]
        )
        for param in sorted(list(unknown_params), key=str.lower):
            self.log.warning('Warning: ignoring unknown parameter "%s" '
                             '(value: %s)' % (param, self.conf[param]))
        for extracheck in getattr(self, '_extraconfchecks', []):
            extracheck(self.conf)
    ...
    def configuration(self):
        classname = repr(self)[1:].split()[0].split('.')[-1]
        return '%s(%s)' % (classname, self._confstring())
